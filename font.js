...
function getMain() {
  ...
  if (!main) throw new Error("Nenhuma conversa aberta (#main não encontrado).");
  return main;
}
...
function findComposer(main) {
  let editable =
    ...
    main.querySelector('#main div[contenteditable="true"]');

  if (!editable) {
    const all = [...main.querySelectorAll('div[contenteditable="true"]')].filter(isVisible);
    editable = all.at(-1) || all[0] || null;
  }

  if (!editable) throw new Error("Campo de digitação não encontrado (contenteditable).");
  return editable;
}

async function setComposerText(editable, text) {
  editable.focus();
...
async function waitForSendButton(main, timeoutMs = 500, pollMs = 50) {
  ...
  while (performance.now() - start < timeoutMs) {
    for (const sel of iconSelectors) {
      const icon = main.querySelector(sel);
...
async function trySendWithEnter(editable) {
...
// ===== Core =====
async function enviarScript(scriptText, delay = 100) { // default: 100ms entre linhas
  ...
  const main = getMain();
  const editable = findComposer(main);

  for (const [i, line] of lines.entries()) {
    console.log(`[${i + 1}/${lines.length}] >`, line);

    await setComposerText(editable, line);

    const sendButton = await waitForSendButton(main, 500, 50); // máx 0,5s de espera
    if (sendButton) {
      sendButton.click();
    } else {
      console.warn("Botão de envio não encontrado; tentando Enter…");
      await trySendWithEnter(editable);
    }

    if (i !== lines.length - 1) {
      await sleep(Math.min(delay, 500)); // nunca mais que 500ms
    }
  }
...
async function enviarScriptVariasVezes(
  scriptText,
  delayEntreLinhas = 100,
  vezes = 3,
  delayEntreExecucoes = 500
) {
  for (let i = 0; i < vezes; i++) {
    console.log(`Execução ${i + 1} de ${vezes}`);
    try {
      const mensagens = await enviarScript(scriptText, delayEntreLinhas);
      console.log(`Execução ${i + 1} finalizada, ${mensagens} mensagens enviadas`);
    } catch (err) {
    ...
    if (i < vezes - 1) {
      await sleep(Math.min(delayEntreExecucoes, 500)); // nunca mais que 500ms
    }
  }
...
enviarScriptVariasVezes(script, 100, 1, 300);